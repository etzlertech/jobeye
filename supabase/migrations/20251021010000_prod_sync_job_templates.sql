-- Migration: Production sync for job_templates table and FK constraint
-- Purpose: Fix PGRST200 error in production by creating missing job_templates table
-- Date: 2025-10-21
--
-- IMPORTANT: This migration is REDUNDANT with 001_v4_core_business_tables.sql
-- It exists for historical production sync purposes and version control reproducibility.
-- The job_templates table and FK are the canonical definitions in 001_v4_core_business_tables.sql.
-- This migration is idempotent (uses IF NOT EXISTS) and safe to run after 001.
--
-- Note: Pre-commit cleanup verification will warn about duplicate table creation - this is expected.
-- Do not delete this file without updating production migration history.
