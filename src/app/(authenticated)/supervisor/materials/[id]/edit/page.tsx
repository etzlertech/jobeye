...
export default function EditMaterialPage() {
  ...
  useEffect(() => {
    const loadItem = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error(data.message || 'Failed to load material');
        }

        const item = data.item;
        setName(item.name || '');
        setDescription(item.description || '');
        setCategory(item.category || 'lumber');
        setQuantity(String(item.current_quantity || 0));
        setUnit(item.unit_of_measure || 'each');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/supervisor/materials/${itemId}`);
    } catch (err) {
  ...
  return (
    <div className="mobile-container">
      ...
      <div className="bottom-actions">
        <button
          type="button"
          onClick={() => router.push(`/supervisor/materials/${itemId}`)}
          disabled={isSubmitting}
