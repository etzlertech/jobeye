 ...
 * Upload and persist task template images
 *
 * @task T009
  ...
  try {
    ...
    if (!context.isSupervisor) {
      return NextResponse.json(
        {
          error: 'Forbidden',
          message: 'Only supervisors can upload template images',
    ...
    console.error('[POST /api/task-templates/[id]/image] Unexpected error', error);
    return NextResponse.json(
      {
  ...
  try {
    ...
    if (!context.isSupervisor) {
      return NextResponse.json(
        {
          error: 'Forbidden',
          message: 'Only supervisors can remove template images',
          code: 'INSUFFICIENT_PERMISSIONS',
        },
        { status: 403 }
      );
    ...
    console.error('[DELETE /api/task-templates/[id]/image] Unexpected error', error);
    return NextResponse.json(
      {
        error: 'UNEXPECTED_ERROR',
        message: error instanceof Error ? error.message : 'Unknown error',
